...
Understood.
Core protocol switched.
Language module: `en_US`.
Outputting final deliverable block.
...
| :--------------: | :------: | :---------: | :----: | :-----: | :-----: |
| **Temperature** |  22.51   |    20.49    | 13.51  |  26.50  |  27.01  |
...
-   For Part 2, please explain how Spark computes the average temperatures using the spark operations that you used in your code (1 point)
    ...
    > Spark first reads the `input.txt` file from HDFS as raw strings into a DataFrame using `spark.read.text`. Then, it uses the `split` function to parse each line by the comma delimiter into two columns: city name and temperature, with the temperature column being cast to a numeric type. The core computation step is `groupBy("city_name")`, which aggregates all rows with the same city name into a single group. Finally, the `avg("temperature")` aggregation function is called on each group to calculate the mean of all temperature values for that city, yielding the final result.
...
-   For Part 3, please explain how Spark computes the page rank using the spark operations that you used in your code (1 point)

    > My PageRank implementation is based on the Spark RDD API. First, the program reads the graph data from HDFS, filters out comment lines, and transforms it into an RDD of `(source_node, destination_node)` link pairs. It then converts the link pairs into an adjacency list format of `(source_node, [list_of_neighbors])` using `groupByKey`, which is crucial for calculating contributions. The initial rank of all nodes is set to 1.0. In the core loop, which runs for 10 iterations: 1. A `join` operation combines the link adjacency list RDD with the current `ranks` RDD. 2. A `flatMap` is used to calculate the contribution of each node to its neighbors (`rank / out_degree`) and generates `(destination_node, contribution_value)` key-value pairs. 3. The `reduceByKey` operation sums up all contributions sent to the same destination node. 4. Finally, `mapValues` applies the PageRank formula `0.15 + 0.85 * sum_of_contributions` to the aggregated contributions, completing one round of rank updates. This process is repeated 10 times to get the final PageRank values.

-   For Part 3, Task 2, explain the custom RDD partitioning you implemented and the changes it brings. (1 point)

    > To optimize performance, I applied custom partitioning to the `links` RDD, which represents the adjacency list. After the `groupByKey` operation, I chained a `partitionBy(N)` call (where N is the number of partitions). This operation uses a hash partitioner to co-locate data with the same key (the source node) in the same physical partition. The primary benefit is a significant improvement in the efficiency of the `join` operation in subsequent iterations. Because the `ranks` RDD is also partitioned using the same key, Spark can avoid a costly full network shuffle during the join. Most of the join computation can be performed locally within each executor, drastically reducing network I/O and shortening the overall job execution time.

-   For Part 3 Task 3, explain the changes after the Worker process is killed. (1 point)

    > When I forcibly killed the Spark Worker process on `vm2` using `kill -9`, the Spark Master node quickly detected the loss of the worker via its heartbeat mechanism. All tasks running on that worker's executor failed immediately. However, the entire Spark application did not crash. Spark's fault-tolerance mechanism was triggered automatically. The Driver process, aware of the DAG of dependencies, identified the RDD partitions whose results were lost due to the worker's death. It then resubmitted the failed tasks and rescheduled them to run on the remaining active worker nodes (in this case, the worker on `vm1`). This recovery process is transparent to the user, and the job eventually completes successfully, although the total execution time is noticeably longer due to the overhead of task re-scheduling and re-computation.
